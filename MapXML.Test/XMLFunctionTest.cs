...
namespace MapXML.Tests
{
    ...
    public class XMLFunctionTests : BaseTestClass
    {
        ...
        public void SimpleLookup()
        {
            ...
            TestBaseContext handler = new TestBaseContext();
        ...
        {
            Stream s = GetTestXML("XMLFunctions");
            TestBaseContext cont = new TestBaseContext();
            cont.Associate<Test_WithLookup>("Tests");
 

            XMLDeserializer xdes = new XMLDeserializer(cont, s, null)
            {
                IgnoreRootNode = false
            };
            xdes.Run();
            //---------------------------//
            XMLSerializer ser = new XMLSerializer(cont, OptionalRootNode:"Tests");
        ...
        public void SerializeLookupFunction()
        {
            ...
            //---------------------------//
            //---------------------------//
            XMLSerializer ser = new XMLSerializer(handler, "Tests", handler.GetResults<Test_WithLookup>().First());
            ser.Run();
        }

        private class TestBaseContext: DefaultHandler
        {
            public override bool Lookup_FromAttributes(IXMLState state, string nodeName, IReadOnlyDictionary<string, string> attributes, Type targetType, out object result)
            {
